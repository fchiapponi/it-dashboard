export async function register() {
  ...
  const { pollAllServers } = await import("@/lib/server-service");
  ...
  cron.schedule(`*/${serverIntervalMin} * * * *`, () => {
    pollAllServers().catch((err) => console.error("[poller] server poll error", err));
  });
  ...
  setTimeout(() => {
    ...
    pollAllServers().catch((err) => console.error("[poller] initial server poll error", err));
  }, 5000);

  console.log(
    `[poller] started — printers every ${printerIntervalMin}min, cameras every ${cameraIntervalMin}min, servers every ${serverIntervalMin}min`,
  );
}
